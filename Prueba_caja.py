...
#variable para la app web
mensajes_copia_appweb = {f"M{i}": [] for i in range(1, 29)}
...
def guardar_mensajes():
    ...
    with lock_orden_mesas: # Proteger el acceso a las variables mientras se guardan
        try:
            ...
            #         SERVER_URL,
            #         json={
            #             "mensajes_por_categoria": mensajes_copia_appweb,
            #             "orden_mesas": orden_mesas
            #         },
            #         timeout=15
            #     )
            #     if r.status_code == 200:
            #         print("✅ Comandas enviadas al servidor en tiempo real")
            #     else:
            #         print(f"⚠️ Error al enviar al servidor: {r.status_code}")
...
def copiar_mensajes_por_categoria():
    with lock_orden_mesas:
        ...
        try:
            with open("mensajes_copia_appweb.json", "w") as f:
                json.dump({"mensajes": mensajes_copia_appweb}, f, indent=2, ensure_ascii=False)
        except Exception as e:
            print(f"Error al guardar mensajes_copia_appweb.json: {e}")

# Función para cargar los mensajes desde un archivo JSON
def cargar_mensajes():
   with lock_orden_mesas:
    global mensajes_por_categoria, orden_mesas_real, orden_mesas, mensajes_copia, mensajes_copia_appweb
    ...
    if os.path.exists("mensajes_copia_appweb.json") and os.path.getsize("mensajes_copia_appweb.json") != 0:
        with open("mensajes_copia_appweb.json", "r") as f:
            datos = json.load(f)
            mensajes_copia_appweb = datos.get("mensajes", {})
    else:
        mensajes_copia_appweb = {f"M{i}": [] for i in range(1, 29)}
        with open("mensajes_copia_appweb.json", "w") as f:
            json.dump({"mensajes": mensajes_copia_appweb}, f)

def cargar_reservas(self):
    """Cargar reservas desde el JSON al iniciar la app."""
    try:
        with open("mensajes_copia_appweb.json", "r") as f:
            datos = json.load(f)
        mensajes_por_categoria_app = datos.get("mensajes", {})
    except FileNotFoundError:
        mensajes_por_categoria_app = {}

    for nombre_mesa, mensajes in mensajes_por_categoria_app.items():
        celda = self.celdas.get(nombre_mesa)
        if not celda:
            continue
        # Revisar si la primera entrada es "R" (reserva)
        if mensajes and mensajes[0] == "R":
            celda.cambiar_color((1, 1, 0.5, 1))  # Amarillo pálido
            if hasattr(celda, 'boton_reservar'):
                celda.boton_reservar.text = "Cancelar Reserva"
                celda.boton_reservar.on_release = lambda btn, c=celda: self.cancelar_reserva(c)
        else:
            # Caso NO reserva → gris
            celda.cambiar_color((0.95, 0.95, 0.95, 1))  # Gris
            if hasattr(celda, 'boton_reservar'):
                celda.boton_reservar.text = "Reservar"
                celda.boton_reservar.on_release = lambda btn, c=celda: self.reservar(c)
...
def procesar_mensaje(message, estado):
    try:
        ...
        match = re.search(r'\b[mb]([1-9]|1[0-9]|2[0-6]|28)\b|\bpa(ra)?\s*llevar\b', text, re.IGNORECASE)
        if not match:
            return
        ...
        mensajes_copia_appweb[categoria].append(nuevo_mensaje)
        nueva_mesa = 0
...
class CeldaMesa(MDCard):
    ...
    def cambiar_color(self, color_rgba):
        self.md_bg_color = color_rgba
...
class TabletApp(MDApp):
    ...
    def mostrar_a_pantalla_completa(self, celda_real):
        ...
        # --- Comprobar si está reservada y cambiar el color a amarillo si lo está ---
        try:
            with open("mensajes_copia_appweb.json", "r") as f:
                datos = json.load(f)
            mensajes_por_categoria_app = datos.get("mensajes", {})
        ...
        # Revisar si la primera entrada es "R" (reserva)
        clave_mesa = f"M{celda_real.numero}"
        mensajes_mesa = mensajes_por_categoria_app.get(clave_mesa, [])

        if mensajes_mesa and mensajes_mesa[0] == "R":
            self.celda_fullscreen_clone.cambiar_color((1, 1, 0.6, 1))  # amarillo claro
        else:
            self.celda_fullscreen_clone.cambiar_color((0.95, 0.95, 0.95, 1))  # gris claro
        ...
        self.boton_finalizar = MDRectangleFlatIconButton(
            text="Completar",
            icon="check",
            theme_text_color="Custom",
            icon_color=(1, 1, 1, 1),      # 
            ...
            size=(dp(300), dp(90)),  # bastante grande
            pos_hint={'right': 0.985, 'center_y': 0.77},  # derecha, centrado vertical
            on_release=lambda x: self.finalizar_comanda(x),
        )
        self.fullscreen_layout.add_widget(self.boton_finalizar)

        # --- Botón: Reservar Mesa ---
        self.boton_reservar = MDRectangleFlatIconButton(
            text="Reservar \n mesa",
            icon="calendar-clock",  # icono de calendario / reserva
            theme_text_color="Custom",
            text_color=(0, 0, 0, 1),
            icon_color=(0, 0, 0, 1),
            line_color=(1, 0.85, 0, 1),   # amarillo borde
            md_bg_color=(1, 0.85, 0, 1),  # amarillo relleno
            size_hint=(None, None),         # 👈 necesario para tamaño fijo
            padding= (dp(5), dp(5), dp(5), dp(5)),
            width=dp(200),                  # 👈 ahora sí funciona
            height=dp(70),
            font_size=dp(18),
            pos_hint={'x': 0.02, 'top': 0.81},
            on_release=lambda btn, c=celda_real: self.reservar_mesa(c),
        )
        # Revisar JSON para determinar si ya está reservada
        nombre_mesa = f"M{celda_real.numero}"
        with open("mensajes_copia_appweb.json", "r") as f:
            mensajes_copia_appweb = json.load(f).get("mensajes", {})

        if "R" in mensajes_copia_appweb.get(nombre_mesa, []):
            self.boton_reservar.text = "Cancelar Reserva"
            self.boton_reservar.icon = "calendar-remove"
        else:
            self.boton_reservar.text = "Reservar mesa"
            self.boton_reservar.icon = "calendar-clock"


        celda_real.boton_reservar = self.boton_reservar
        self.fullscreen_layout.add_widget(self.boton_reservar)
    ...
    def finalizar_comanda(self, instance, *args):
        """Eliminar la comanda de la celda actual y dar por finalizada."""
        original = [f"M{i}" for i in range(1, 29)]
        global orden_mesas, orden_mesas_real, mensajes_por_categoria, mensajes_copia_appweb

        if not hasattr(self, "celda_fullscreen_real") or self.celda_fullscreen_real is None:
            print("⚠️ No hay celda en pantalla completa para finalizar")
            return
        ...
        if nombre_mesa in orden_mesas:
            ...
            # Limpiar mensajes y actualizar
            mensajes_por_categoria[nombre_mesa] = []
            orden_mesas.remove(nombre_mesa)

        # Limpiar mensajes también en la copia de appweb
        mensajes_por_categoria[nombre_mesa] = []
        mensajes_copia_appweb[nombre_mesa] = []
        self.celda_fullscreen_real.label_mensajes.text = ""  # limpiar celda real

        # Limpiar clone en fullscreen (si existe)
        if hasattr(self, "celda_fullscreen_clone") and self.celda_fullscreen_clone:
            self.celda_fullscreen_clone.label_mensajes.text = ""
        ...
        # Restaurar reservas y vista
        cargar_reservas(self)
        self.restaurar_vista()
        # 👇 Reordenar explícitamente las celdas en el grid
        if self.grid and orden_mesas_real:
            self.grid.clear_widgets()
            for mesa in orden_mesas_real:
                self.grid.add_widget(self.celdas[mesa])
    ...
    def reservar_mesa(self, celda):
        """Marca la celda como reservada cambiando su color."""
        global mensajes_copia_appweb
        nombre_mesa = f"M{celda.numero}"

        # Cargar datos actuales
        with open("mensajes_copia_appweb.json", "r") as f:
            mensajes_copia_appweb = json.load(f).get("mensajes", {})

        if celda.boton_reservar.text == "Reservar mesa":
            celda.boton_reservar.text = "Cancelar Reserva"

            # Cambiar color en la vista normal
            celda.cambiar_color((1, 1, 0.5, 1))  # Amarillo pálido

            # Cambiar color en la vista fullscreen (si está activa)
            if hasattr(self, "celda_fullscreen_clone") and self.celda_fullscreen_clone:
                self.celda_fullscreen_clone.cambiar_color((1, 1, 0.5, 1))

            # Guardar en JSON
            mensajes_copia_appweb[nombre_mesa].insert(0, "R")
            with open("mensajes_copia_appweb.json", "w") as f:
                json.dump({"mensajes": mensajes_copia_appweb}, f)

        elif celda.boton_reservar.text == "Cancelar Reserva":
            celda.boton_reservar.text = "Reservar mesa"

            # Restaurar color original en la vista normal
            celda.cambiar_color((0.95, 0.95, 0.95, 1))

            # Restaurar color original en fullscreen también
            if hasattr(self, "celda_fullscreen_clone") and self.celda_fullscreen_clone:
                self.celda_fullscreen_clone.cambiar_color((0.95, 0.95, 0.95, 1))

            # Actualizar JSON
            if "R" in mensajes_copia_appweb.get(nombre_mesa, []):
                mensajes_copia_appweb[nombre_mesa].remove("R")
            with open("mensajes_copia_appweb.json", "w") as f:
                json.dump({"mensajes": mensajes_copia_appweb}, f)
    ...
    def restaurar_vista(self):
        ...
        # 👇 IMPORTANTE: Resetear flag de la celda real ANTES de perder la referencia
        if self.celda_fullscreen_real is not None:
            print(f"[DEBUG] Restaurando fullscreen de celda {self.celda_fullscreen_real.numero}")
            self.celda_fullscreen_real.fullscreen = False
    ...
    def resetear_celdas(self, *args):
        def confirmar_reset(instance):
            ...
            for i in range(1, 29):
                ...
                celda.cambiar_color((0.95, 0.95, 0.95, 1))  # Restaurar color original
